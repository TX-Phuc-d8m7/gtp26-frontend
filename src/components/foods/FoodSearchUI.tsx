...
import { Search, SlidersHorizontal, ArrowLeft, HeartPulse, ChefHat, Info, Tag } from "lucide-react";
import { useRouter } from "next/navigation";
import { MOCK_FOODS, Food } from "@/data/mockFoods";
...
export default function FoodSearchUI({ onClose }: FoodSearchUIProps = {}) {
  ...
  const filteredFoods = useMemo(() => {
    return MOCK_FOODS.filter(food => {
      ...
      const matchCat = selectedCategory ? food.categories.includes(selectedCategory) : true;
  ...
  return (
    <div className="flex flex-col h-full w-full relative">
        ...
        <div className="max-w-5xl mx-auto">
          {filteredFoods.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-muted-foreground">Hãy thử với từ khóa khác hoặc bỏ chọn bộ lọc.</p>
            </div>
          ) : (
      ...
      {/* Detail Sheet/Modal */}
      <Sheet open={selectedFood !== null} onOpenChange={(open) => !open && setSelectedFood(null)}>
            ...
            <div className="flex flex-col min-h-full">
              ...
              <div className="relative h-64 w-full bg-muted shrink-0">
                ...
                <div className="absolute bottom-4 left-6 right-6">
                  <SheetTitle className="text-3xl font-bold text-foreground mb-1">{selectedFood.name}</SheetTitle>
                  <p className="text-primary font-semibold">{selectedFood.priceRange}</p>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-4">
                    {selectedFood.categories.map(cat => (
                      <span key={cat} className="inline-flex items-center gap-1 rounded-full bg-secondary px-3 py-1 text-xs font-medium text-secondary-foreground">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 text-center">
                    <div className="bg-muted p-2 rounded-lg">
                      <div className="text-lg font-bold">{selectedFood.nutrition.calories}</div>
                      <div className="text-xs text-muted-foreground">Kcal</div>
                    </div>
                    <div className="bg-muted p-2 rounded-lg">
                      <div className="text-lg font-bold">{selectedFood.nutrition.protein}g</div>
                      <div className="text-xs text-muted-foreground">Đạm</div>
                    </div>
                    <div className="bg-muted p-2 rounded-lg">
                      <div className="text-lg font-bold">{selectedFood.nutrition.carbs}g</div>
                      <div className="text-xs text-muted-foreground">Tinh bột</div>
                    </div>
                    <div className="bg-muted p-2 rounded-lg">
                      <div className="text-lg font-bold">{selectedFood.nutrition.fat}g</div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedFood.ingredients.map(ing => (
                      <span key={ing} className="px-3 py-1.5 bg-background border border-border rounded-md text-sm">
                        {ing}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4">
                    ...
                    {selectedFood.allergies && selectedFood.allergies.length > 0 && (
                      <p className="text-sm text-foreground mb-2">
                        <strong className="text-red-600 dark:text-red-400">Dị ứng:</strong> Có chứa {selectedFood.allergies.join(", ")}.
                      </p>
                    )}
                    {selectedFood.medicalAdvice && (
                      <p className="text-sm text-foreground">
                        <strong className="text-red-600 dark:text-red-400">Khuyên dùng:</strong> {selectedFood.medicalAdvice}
                      </p>
                    )}
                  </div>
                )}
