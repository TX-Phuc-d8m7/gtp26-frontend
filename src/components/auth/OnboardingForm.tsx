...
import { onboardingSchema, OnboardingFormData, ALLERGY_OPTIONS, FAVORITE_OPTIONS, DISLIKE_OPTIONS } from "@/schemas/onboarding";
...
export default function OnboardingForm() {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-6 sm:p-8 rounded-2xl bg-card border border-border shadow-lg">
      <div className="mb-8">
        <h2 className="text-3xl font-bold tracking-tight text-foreground mb-2">Cá nhân hóa trải nghiệm</h2>
        <p className="text-muted-foreground">Chọn sở thích ẩm thực để chúng tôi gợi ý món ăn phù hợp nhất với bạn.</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="space-y-2">
          <label className="text-base font-semibold text-foreground flex items-center gap-2">
            Dị ứng thực phẩm 
            <span className="text-xs font-normal text-muted-foreground bg-secondary px-2 py-0.5 rounded-full">Không bắt buộc</span>
          </label>
          <p className="text-sm text-muted-foreground">Chọn các nguyên liệu bạn bị dị ứng (nếu có).</p>
          <Controller
            name="allergies"
            control={control}
            render={({ field }) => (
        ...
        <div className="space-y-2">
          <label className="text-base font-semibold text-foreground flex items-center gap-2">
            Món ăn yêu thích
            <span className="text-xs font-normal text-muted-foreground bg-secondary px-2 py-0.5 rounded-full">Nhiều lựa chọn</span>
          </label>
          <p className="text-sm text-muted-foreground">Bạn thường thích ăn những loại thức ăn nào?</p>
          <Controller
            name="favorites"
            control={control}
            render={({ field }) => (
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">Những nguyên liệu hoặc món bạn không bao giờ muốn ăn.</p>
          <Controller
            name="dislikes"
            control={control}
            render={({ field }) => (
